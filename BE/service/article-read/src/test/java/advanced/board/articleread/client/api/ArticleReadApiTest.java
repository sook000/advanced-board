...
import advanced.board.articleread.service.response.ArticleReadPageResponse;
...
public class ArticleReadApiTest {
    ...
    void readAllTest() {
        ArticleReadPageResponse response1 = articleReadRestClient.get()
                .uri("/v1/articles?boardId=%s&page=%s&pageSize=%s".formatted(1L, 4000L, 5))
                .retrieve()
    ...
    void readAllInfiniteScrollTest() {
        List<ArticleReadResponse> response1 = articleReadRestClient.get()
                .uri("/v1/articles/infinite-scroll?boardId=%s&pageSize=%s&lastArticleId=%s".formatted(1L, 5L, 211827085509230592L))
                .retrieve()
